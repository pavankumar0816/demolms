        ...
        <Table style={{ border: '3px solid black' }}>
          <TableHead>
            <TableRow style={{ border: '3px solid black' }}>
              ...
              <TableCell style={{ fontWeight: 'bold', borderRight: '3px solid black', borderBottom: '3px solid black' }}>Assessment File</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {events.length > 0 ? (
                  ...
                  <TableCell style={{ borderRight: '3px solid black', borderBottom: '3px solid black' }}>
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                      {event.file.endsWith('.jpg') || event.file.endsWith('.jpeg') || event.file.endsWith('.png') ? (
                        <img src={`${config.url}/assessmentfile/${event.file}`} alt="Event" style={{ width: '150px', height: '150px', borderRadius: '5px' }} />
                      ) : (
                        <Button variant="contained" color="primary" component="a" href={`${config.url}/assessmentfile/${event.file}`} style={{ marginLeft: 10 }}>Download File</Button>
                      )}
                    </div>
                  </TableCell>
                </TableRow>
